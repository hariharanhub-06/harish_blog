...
export default function FormResponsePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        <div className="min-h-screen relative font-sans text-gray-900 selection:bg-primary/20 bg-black">
            ...
            <div className="min-h-screen selection:bg-primary/20 backdrop-blur-3xl pb-20">
                ...
                <form onSubmit={e => isNextSubmit ? handleSubmit(e) : e.preventDefault()} className="max-w-3xl mx-auto space-y-8 relative z-10 pb-24">
                    ...
                    {currentQuestions.map((q) => (
                        q.type === 'section_header' ? (
                            <div key={q.id || Math.random()} className="bg-primary/5 border border-primary/20 rounded-[2.5rem] p-8 sm:p-12 text-center my-8 shadow-sm relative overflow-hidden">
                                <div className="absolute top-0 left-0 w-full h-2" style={{ backgroundColor: themeBg }}></div>
                                <h2 className="text-3xl font-black" style={{ color: themeBg }}>{q.questionText}</h2>
                                {q.options?.[0] && <p className="text-gray-600 font-medium mt-3">{q.options[0]}</p>}
                            </div>
                        ) : (
                            <div key={q.id || Math.random()} className="bg-white p-8 sm:p-12 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-gray-100 hover:border-gray-200 transition-all duration-300 relative group overflow-hidden">
