...
// --- Agile Scrum Command Center ---

export const agileProjects = pgTable("agile_projects", {
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
  name: text("name").notNull(),
  key: text("key").notNull().unique(), // e.g., "PROJ", "APP"
  description: text("description"),
  lead: text("lead"), // Simple text for assignee
  avatarUrl: text("avatar_url"),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});
...
export const agileWorkflows = pgTable("agile_workflows", {
  ...
  // statusOrder stores the columns and their order: ["To Do", "In Progress", "Code Review", "Done"]
  statusOrder: jsonb("status_order").notNull().$default(() => (["To Do", "In Progress", "Done"])),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});
...
export const agileEpics = pgTable("agile_epics", {
  ...
  title: text("title").notNull(),
  description: text("description"),
  color: text("color").default("#0052CC"), // Jira blue
...
export const agileIssues = pgTable("agile_issues", {
  ...
  title: text("title").notNull(),
  description: text("description"),
  type: text("type").notNull().default("story"), // story, task, bug, subtask
  ...
  position: real("position").notNull().default(0), // For ranking in backlog/board
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});
...
export const agileMeetings = pgTable("agile_meetings", {
  ...
  projectId: text("project_id").notNull(),
  sprintId: text("sprint_id"),
  type: text("type").notNull(), // daily, planning, retro
  date: timestamp("date").defaultNow(),
  // content stores minutes, action items, retro notes
  content: jsonb("content").$default(() => ({})),
  mood: integer("mood").default(5), // Team mood tracking 1-10
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});
...
export const agileDeploymentLogs = pgTable("agile_deployment_logs", {
  ...
  bugsFixed: jsonb("bugs_fixed").$default(() => ([])),
  featuresShipped: jsonb("features_shipped").$default(() => ([])),
  createdAt: timestamp("created_at").defaultNow(),
});

// --- Agile Relations ---

export const agileProjectRelations = relations(agileProjects, ({ many }) => ({
  epics: many(agileEpics),
  sprints: many(agileSprints),
  issues: many(agileIssues),
  workflows: many(agileWorkflows),
}));

export const agileEpicRelations = relations(agileEpics, ({ one, many }) => ({
  project: one(agileProjects, {
    fields: [agileEpics.projectId],
    references: [agileProjects.id],
  }),
  issues: many(agileIssues),
}));

export const agileSprintRelations = relations(agileSprints, ({ one, many }) => ({
  project: one(agileProjects, {
    fields: [agileSprints.projectId],
    references: [agileProjects.id],
  }),
  issues: many(agileIssues),
  meetings: many(agileMeetings),
}));

export const agileIssueRelations = relations(agileIssues, ({ one, many }) => ({
  project: one(agileProjects, {
    fields: [agileIssues.projectId],
    references: [agileProjects.id],
  }),
  epic: one(agileEpics, {
    fields: [agileIssues.epicId],
    references: [agileEpics.id],
  }),
  sprint: one(agileSprints, {
    fields: [agileIssues.sprintId],
    references: [agileSprints.id],
  }),
  parent: one(agileIssues, {
    fields: [agileIssues.parentId],
    references: [agileIssues.id],
    relationName: "subtasks",
  }),
  subtasks: many(agileIssues, {
    relationName: "subtasks",
  }),
}));
